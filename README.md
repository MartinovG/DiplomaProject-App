...
│   ├── src/
│   │   ├── index.ts          API server (envs, scale, history endpoints)
...
| `GET` | `/api/envs` | List all Kubernetes namespaces with deployment status |
| `POST` | `/api/scale` | Sleep (`replicas: 0`) or wake (`replicas: 1`) a deployment |
| `GET` | `/api/history` | Audit log of all sleep/wake actions |
...
Schema: a single `AuditLog` table tracking sleep/wake actions with timestamps.
